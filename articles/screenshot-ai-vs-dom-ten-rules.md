...
1. 対照（違反と非違反の両方）がある 60 組で **GPT 54/60・Gemini 41/60（不明 3）**。ただし違反は 14 組だけなので、**全部 NO と答えるだけで 46/60**。GPT は全体で上回るが計測型は 18/24 で基準の 19/24 を下回り、Gemini は全体でも下回る（2026-09-26 追記）。
...
| 基準: 全部 NO と答える | 対照あり（R1〜R4） | 60 | 100% | **46/60＝77%**（計測型 19/24） | 0% | 100% |
